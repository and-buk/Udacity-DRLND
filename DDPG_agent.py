...
UPDATE_FREQ = 10        # how many times update the network
...
class Agent():
    ...
    def __init__(self, state_size, action_size, num_agents, seed):
        ...
        # Get evenly sigma values within a given interval (high, low, bins)
        self.sigma_grid = np.linspace(0.2, 0.08, 20)
        # Get evenly score values within a given interval (low, high, bins)
        self.score_grid = np.linspace(30, 40, 20)
    ...
    def act(self, state, score=None, add_noise=True):
        ...
        if add_noise and score is not None:
            # After +30 score set the value of sigma according to uniformly-spaced grid  
            if score >= 30:
                score_grid_idx = np.digitize(score, self.score_grid)
                self.noise.set_sigma(self.sigma_grid[score_grid_idx-1])
            else:
                self.noise.sigma = 0.2  
...
class OUNoise:
    ...
    def sample(self):
        ...
        x = self.state
        dx = self.theta * (self.mu - x) + self.sigma * (np.random.random(x.shape)-0.5)           
        self.state = x + dx
        return self.state
